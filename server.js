...
// Build a multi-tab availability check workbook (raw inventory, no allocation)
async function buildWorkbookFromAvailabilityData({
  requestedSeen,        // [{ handle, size, requestedQty }]
  availabilitySeen,     // [{ handle, size, requestedQty, availabilityByLocationId }]
  metaByHandle,         // { handle: { title, imageUrl } }
  locationIdToName,     // { locId: name }
  locationIdsInOrder,   // [locId, ...]
  customer,
  ...
  // Build availability maps: total and per-location (capped at requested)
  const totalAvailMap = new Map();   // handle -> size -> min(requested, sum of available)
  const locAvailMap = new Map();     // locId -> handle -> size -> available qty

  for (const entry of availabilitySeen || []) {
    const { handle, size, requestedQty, availabilityByLocationId } = entry;
    if (!handle || !size) continue;

    let sumAvail = 0;
    for (const locId of locationIdsInOrder) {
      const avail = availabilityByLocationId?.[locId] || 0;
      if (avail <= 0) continue;
      sumAvail += avail;

      if (!locAvailMap.has(locId)) locAvailMap.set(locId, new Map());
      addQtyToMap(locAvailMap.get(locId), handle, size, avail);
    }

    // Total available is capped at requested
    const fulfillable = Math.min(requestedQty || 0, sumAvail);
    addQtyToMap(totalAvailMap, handle, size, fulfillable);
  }

  // Total Warehouse (Bogota + Warehouse) availability
  const totalWarehouseMap = new Map();
  for (const [locId, handleMap] of locAvailMap.entries()) {
    const locName = locationIdToName[locId] || "";
    if (locName !== "Bogota" && locName !== "Warehouse") continue;
    for (const [handle, sizeMap] of handleMap.entries()) {
      if (!totalWarehouseMap.has(handle)) totalWarehouseMap.set(handle, makeHandleSizeMapEmpty());
      const tgt = totalWarehouseMap.get(handle);
      for (const s of SIZES) tgt.set(s, (tgt.get(s) || 0) + Number(sizeMap.get(s) || 0));
    }
  }

  const wb = new ExcelJS.Workbook();
  wb.creator = "Wholesale Importer (Availability Check)";
  wb.created = new Date();
  ...
  await addSheet("Requested", "ALL REQUESTED", requestedHandleMap);
  await addSheet("Available Total", "FULFILLABLE (capped at requested)", totalAvailMap);
  await addSheet("Total Warehouse", "Bogota + Warehouse", totalWarehouseMap);

  const locIds = Array.isArray(locationIdsInOrder) && locationIdsInOrder.length
    ? locationIdsInOrder
    : Array.from(locAvailMap.keys());

  for (const locId of locIds) {
    const locName = locationIdToName[locId] || locId;
    const handleMap = locAvailMap.get(locId) || new Map();
    await addSheet(locName, locName + " (available)", handleMap);
  }

  const bufAvail = await wb.xlsx.writeBuffer();
  return Buffer.from(bufAvail);
}
...
// ----------------- Availability check (no allocation, just raw inventory) -----------------
async function runAvailabilityCheck(req) {
  const { locationIdsJson } = req.body;
  ...
  const metaByHandle = {};
  const requestedSeen = [];
  const availabilitySeen = []; // { handle, size, requestedQty, availabilityByLocationId }
  ...
  for (const item of requested) {
    ...
    if (!product) {
      missingHandles.push(item.handle);
      continue;
    }
    ...
    for (const size of SIZES) {
      ...
      totalRequested += qty;
      requestedSeen.push({ handle: item.handle, size, requestedQty: qty });

      const variant = findVariantForSize(product.variants.nodes, size);
      if (!variant) {
        availabilitySeen.push({
          handle: item.handle, size, requestedQty: qty,
      ...
      const availByLoc = {};
      for (const locId of locationIdsInOrder) {
        const avail = await getAvailableAtLocation(variant.inventoryItem.id, locId);
        availByLoc[locId] = Math.max(0, avail);
      }

      const totalAtAllLocs = Object.values(availByLoc).reduce((s, v) => s + v, 0);
      totalAvailable += Math.min(qty, totalAtAllLocs);

      availabilitySeen.push({
        handle: item.handle, size, requestedQty: qty,
        availabilityByLocationId: availByLoc
      });
    }
  }
  ...
  return {
    ...
    totalRequested,
    totalAvailable,
    totalShort: totalRequested - totalAvailable
  };
}
...
app.get("/api/report.xlsx", async (req, res) => {
  try {
    ...
    let buf;
    if (run.isAvailabilityCheck && run.metaByHandle && run.availabilitySeen) {
      // Availability check — raw inventory report
      buf = await buildWorkbookFromAvailabilityData({
    ...
    const suffix = run.isAvailabilityCheck ? "availability-report"
      : run.orderIds && run.orderIds.length > 0 ? "fulfillment-report" : "preview-report";
...
// Build a browser-printable HTML report from availability check data
function buildAvailabilityPrintHtml(run) {
  function esc(s) {
    return String(s || "")
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  const locIdToName = run.locationIdToName || {};
  const locIds = run.locationIdsInOrder || [];
  const SIZES_LOCAL = ["XXS", "XS", "S", "M", "L", "XL", "XXL"];
  ...
  // Build per-location availability maps and total available
  const availMap = {};    // handle -> size -> { locId: qty }
  const totalAvail = {};  // handle -> size -> fulfillable (capped at requested)
  for (const entry of run.availabilitySeen || []) {
    const { handle, size, requestedQty, availabilityByLocationId } = entry;
    if (!handle || !size) continue;
    if (!availMap[handle]) availMap[handle] = {};
    availMap[handle][size] = availabilityByLocationId || {};

    let sum = 0;
    for (const locId of locIds) sum += (availabilityByLocationId?.[locId] || 0);
    if (!totalAvail[handle]) totalAvail[handle] = {};
    totalAvail[handle][size] = Math.min(requestedQty || 0, sum);
  }
  ...
  // Summary totals
  let totalReq = 0, totalFulfillable = 0;
  for (const { requestedQty } of run.requestedSeen || []) totalReq += Number(requestedQty || 0);
  for (const h of allHandles) {
    for (const s of SIZES_LOCAL) totalFulfillable += (totalAvail[h]?.[s] || 0);
  }
  const totalShort = totalReq - totalFulfillable;

  // Location headers
  const locHeaders = locIds.map(id => esc(locIdToName[id] || id));

  function sizeRow(handle, size) {
    const req = reqMap[handle]?.[size] || 0;
    const fulfillable = totalAvail[handle]?.[size] || 0;
    if (req === 0) return "";
    const short = req - fulfillable;
    const locCells = locIds.map(id =>
      `<td>${availMap[handle]?.[size]?.[id] || 0}</td>`
    ).join("");
    return `<tr>
      <td class="size-lbl">${esc(size)}</td>
      <td>${req}</td>
      <td>${fulfillable}</td>
      ${short > 0 ? `<td class="dropped">${short}</td>` : "<td>—</td>"}
      ${locCells}
    </tr>`;
  }
  ...
  const productSections = allHandles.map(handle => {
    const title = esc(meta[handle]?.title || handle);
    const rows = SIZES_LOCAL.map(s => sizeRow(handle, s)).filter(Boolean).join("");
    if (!rows) return "";
    return `
      <div class="product-block">
        <div class="product-title">${title} <span class="handle-lbl">${esc(handle)}</span></div>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Availability Report${customer ? " — " + customer : ""}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; font-size: 13px; color: #111; padding: 24px; max-width: 960px; margin: 0 auto; }
    h1 { font-size: 20px; margin-bottom: 4px; }
    ...
    .pill { background: #f2f2f2; border-radius: 20px; padding: 5px 14px; font-size: 13px; font-weight: 600; }
    .pill.drop { background: #ffe0e0; color: #b00; }
    .pill.avail { background: #e0ffe0; color: #060; }
    .product-block { margin-bottom: 28px; break-inside: avoid; }
    .product-title { font-weight: 700; font-size: 14px; margin-bottom: 6px; }
    .handle-lbl { font-weight: 400; color: #777; font-size: 12px; }
    .product-table { width: 100%; border-collapse: collapse; }
...
<body>
  <button class="print-btn" onclick="window.print()">Print / Save as PDF</button>
  <h1>Inventory Availability Report${customer ? " — " + customer : ""}</h1>
  <div class="meta">
    ${fileName ? `<div><b>File:</b> ${fileName}</div>` : ""}
    ${notes ? `<div><b>Notes:</b> ${notes}</div>` : ""}
    <div><b>Date:</b> ${dateStr}</div>
    <div><b>Mode:</b> Availability Check (no order created)</div>
  </div>
  <div class="summary">
    <span class="pill">Requested: ${totalReq}</span>
    <span class="pill avail">Fulfillable: ${totalFulfillable}</span>
    ${totalShort > 0 ? `<span class="pill drop">Short: ${totalShort}</span>` : `<span class="pill">Short: 0</span>`}
    ${locHeaders.map((n, i) => {
      let locTotal = 0;
      for (const h of allHandles) for (const s of SIZES_LOCAL) locTotal += (availMap[h]?.[s]?.[locIds[i]] || 0);
      return `<span class="pill">${n}: ${locTotal}</span>`;
    }).join("")}
  </div>
  ${productSections || "<p>No products in this run.</p>"}
...
// Availability check: query raw inventory at each location (no order created)
app.post("/api/availability-check", upload.single("file"), async (req, res) => {
  try {
    const result = await runAvailabilityCheck(req);

    const runId = saveRun({
      isAvailabilityCheck: true,
      locationIdToName: result.locationIdToName,
      requestedSeen: result.requestedSeen,
      availabilitySeen: result.availabilitySeen,
      metaByHandle: result.metaByHandle,
      locationIdsInOrder: result.locationIdsInOrder,
      customer: result.customer,
      notes: result.notes,
...
// Availability check print preview
app.get("/api/availability-print", (req, res) => {
  try {
    const runId = String(req.query.runId || "");
    const run = getRun(runId);
    if (!run) {
      return res.status(400).send("<h2>Invalid or expired run ID. Run Availability Check again, then print.</h2>");
    }
    const html = buildAvailabilityPrintHtml(run);
    res.setHeader("Content-Type", "text/html; charset=utf-8");
    res.send(html);
  } catch (e) {
    res.status(500).send(`<h2>Error generating availability report: ${String(e?.message || e)}</h2>`);
  }
});
